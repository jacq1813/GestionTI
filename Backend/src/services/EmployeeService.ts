...
export const getTechEmployees = async () => {
    try {
        const [rows] = await connection.query('SELECT e.ID_Emp,e.Nombre,e.ApellidoPat,e.ApellidoMat,e.Fecha_Nac,e.Num_tel, te.Nombre FROM empleado e inner join tipoempleado te on e.ID_TipEmp = te.ID_TipEmp where te.ID_TipEmp = 3');
        return rows;
