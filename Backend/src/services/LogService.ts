...
export const getLogsDetails = async () => {
    try {
        const [rows] = await connection.query(`SELECT bi.Id_Bitacora, bi.Folio_Incidencia, bi.Fecha_cambio, bi.Accion,bi.Descripcion AS 'Descripcion Bitacora',Estado_anterior,Estado_nuevo,CONCAT(E3.Nombre,' ',E3.ApellidoPat,' ', E3.ApellidoMat) AS 'Responsable Bitacora',i.Descripcion as 'Descripcion Incidencia', i.Fecha as 'Fecha Incidencia', i.Prioridad, i.Hora, ed.Nombre as 'Edificio', a.Nombre as 'Aula',ta.Nombre AS 'Tipo Aula', CONCAT(E.Nombre, ' ', E.ApellidoPat, ' ', E.ApellidoMat) AS 'Emisor', CONCAT(E2.Nombre, ' ', E2.ApellidoPat, ' ', E2.ApellidoMat) AS 'Receptor', eq.Version  FROM bitacoraincidencias bi
                    ...
                    INNER JOIN empleado e3 ON e3.ID_Emp = bi.ID_Emp
...
export const addLog = async (log: LogNew) => {
    try {
        const [rows] = await connection.query('INSERT INTO bitacoraincidencias (Folio_Incidencia, Fecha_Cambio, Accion, Descripcion, Estado_anterior, Estado_nuevo,ID_Emp) values (?,?,?,?,?,?,?)', [log.Folio_Incidencia, log.Fecha_Cambio, log.Accion, log.Descripcion, log.Estado_anterior, log.Estado_nuevo, log.ID_Emp]);
        return rows;
    } catch (error) {
